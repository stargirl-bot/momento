...
- Read `@_build_plan/milestones/1-bookmarks-search-filter/milestone-log.md` to understand what was built in milestone 1.
...
4. When complete, write a `milestone-log.md` in this folder (`_build_plan/milestones/2-ai-summarization/milestone-log.md`). Structure it as follows:
   - **Start with a `## What's new in the app` section at the very top.** A concise, human-readable, bulleted list of the main user-facing features added in this milestone — written so a non-technical reviewer can see at a glance what new things to expect in the app.
   - Then include implementation detail sections for the next milestone's agent to reference:
     - What was built (files created, models added, routes added, jobs added, etc.)
